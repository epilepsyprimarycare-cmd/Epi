...
class CDSIntegration {
  ...
  applyPsychosocialScreening(analysis, patientContext) {
    ...
    // Determine if screening is due (biannual = every 180 days)
    ...
    let daysSinceLastScreening = 365; // Default: assume due if never done
    
    if (lastScreeningDate) {
      try {
        const lastDate = new Date(lastScreeningDate);
        if (!isNaN(lastDate.getTime())) {
          const now = new Date();
          daysSinceLastScreening = Math.round((now - lastDate) / (24*60*60*1000));
        }
      } catch (e) {
        window.Logger.warn('Error parsing last screening date:', lastScreeningDate, e);
        daysSinceLastScreening = 365; // Fallback: assume due
      }
    }
    
    const screeningDue = daysSinceLastScreening >= 180; // 6 months = biannual
    
    // CRITICAL: Only show if screening is actually due (not at every visit)
    if (!isRoutineReview || !screeningDue) {
      // Skip screening if not routine review OR not yet due
      return;
    }
    ...
    analysis.prompts.push({
      ...
      text: `Routine screening for anxiety and depression is recommended biannually (every 6 months) for adults with epilepsy. Last screening: ${lastScreeningDate ? new Date(lastScreeningDate).toLocaleDateString() : 'Never'}. Days since last screening: ${daysSinceLastScreening}. Please administer the validated Neurological Disorders Depression Inventory for Epilepsy (NDDI-E). Screening is associated with improved patient quality outcomes (OR 2.97 for documentation-related interventions).`,
        ...
        rationale: 'Biannual psychosocial screening (every 6 months) improves care quality with documented evidence (OR 2.97). Depression prevalence in epilepsy is 23-30%, significantly higher than general population. Screening is due based on 180+ days since last assessment.',
        nextSteps: [
          ...
          'Next screening will be due in 6 months from today'
        ],
