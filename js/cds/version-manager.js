...
class CDSVersionManager {
  ...
  async initialize() {
    try {
      console.log('Initializing CDS Version Manager...');
      
      // Get CDS configuration
      const config = await window.cdsApi.getConfig();
      if (!config) {
        console.log('CDS configuration not available - this is normal if CDS is not configured');
      ...
      if (config.enabled === false) {
        console.log('CDS is disabled in configuration');
        return;
      }
      
      // Get KB metadata
      this.kbMetadata = await window.cdsApi.getKnowledgeBaseMetadata();
      console.log('CDS Version Manager: Received KB metadata:', this.kbMetadata);
      if (!this.kbMetadata) {
        console.log('CDS KB metadata not available - CDS features may be limited');
      ...
      console.log('CDS Version Manager initialized:', {
      ...
      console.error('Error initializing CDS Version Manager:', error);
    }
  }
  ...
  checkVersionCompatibility() {
    ...
    console.log('CDS Version Manager: Checking compatibility - KB version:', version, 'Min required:', this.minRequiredVersion, 'Version data:', versionData);
    ...
    if (this.kbMetadata && this.kbMetadata.status === 'success') {
      console.log('CDS Version Manager: KB metadata available, marking as compatible regardless of version');
      this.isCompatible = true;
    }
    
    if (!this.isCompatible && version !== '0.0.0') {
      console.warn(`CDS KB version ${version} is not compatible. Minimum required: ${this.minRequiredVersion}`);
