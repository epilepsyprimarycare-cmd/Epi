...
window.downloadAllPatientsCsv = async function() {
    console.log('[APP] downloadAllPatientsCsv called (forward declaration)');
    ...
    try {
        ...
        console.error('Failed to export patients CSV:', e);
    }
};
    ...
    page.forEach(p => {
        ...
        patientCard.addEventListener('click', () => {
            if (typeof window.showPatientDetails === 'function') {
                ...
                console.error('[APP] showPatientDetails is not available. Script might have crashed or not loaded completely.');
            }
        });
...
function initializeAllCharts() {
    ...
    renderIfExists(renderPieChart, 'phcChart', 'phcChart', 'Facility Distribution', activePatients.map(p => p.PHC));
    renderIfExists(renderBarChart, 'areaChart', 'areaChart', 'Facility Patient Distribution', activePatients.map(p => p.PHC));
    ...
    if (document.getElementById('treatmentSummaryTable')) {
        try { renderTreatmentSummaryTable(); } catch (e) { window.Logger.warn('renderTreatmentSummaryTable failed', e); }
    }
    ...
    console.log('[APP] script.js reached line 4000');
...
console.log('[APP] script.js reached line 7700');
/**
...
console.log('[APP] About to define showPatientDetails function');
...
console.log('[APP] showPatientDetails function available globally');
...
function attachGlobalActionListeners() {
    console.log('attachGlobalActionListeners called');
    ...
    map.forEach(([id, handlerName]) => {
        ...
        if (el.tagName === 'INPUT' && el.type === 'checkbox') {
            ...
            el.addEventListener('click', (ev) => {
                ev.preventDefault();
                console.log(`Button clicked: ${id}, calling handler: ${handlerName}`);
...
async function downloadAllPatientsCsv() {
    ...
    console.log('downloadAllPatientsCsv called');
    try {
        if (!Array.isArray(window.patientData) || window.patientData.length === 0) {
            showNotification('No patient data loaded. Please refresh the data first.', 'warning');
...
console.log('[APP] script.js execution finished');
