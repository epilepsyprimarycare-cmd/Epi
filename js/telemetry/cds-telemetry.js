...
class CDSTelemetry {
   ...
   * @param {string} eventType - acknowledge, snooze, dismiss
  ...
  flushQueueSync() {
    ...
    try {
      ...
      console.warn('Synchronous telemetry flush failed:', error);
    }
  }
  ...
  async sendEventsBatch(events, attempt = 1) {
    try {
      ...
      console.log(`Telemetry: Successfully sent ${events.length} events`);
      return true;
    } catch (error) {
      console.warn(`Telemetry send attempt ${attempt} failed:`, error);
  ...
  loadPersistedQueue() {
    try {
      ...
      console.warn('Failed to load persisted telemetry queue:', error);
    }
  }
  ...
  clearPersistedQueue() {
    try {
      ...
      console.warn('Failed to clear persisted telemetry queue:', error);
    }
  }
