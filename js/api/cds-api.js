...
class CDSApiClient {
  constructor() {
    ...
    window.addEventListener('online', () => {
      this.isOnline = true;
      console.log('CDS API client: Connection restored');
    });

    window.addEventListener('offline', () => {
      this.isOnline = false;
      console.log('CDS API client: Connection lost');
    });
  }
  ...
  async getConfig() {
    try {
      ...
      if (result.status !== 'success') {
        console.error('Failed to get CDS config:', result.message);
        return null;
      }
      ...
      console.error('Error fetching CDS config:', error);
      return null;
    }
  }
  ...
  async setConfig(config) {
    try {
      ...
      const result = await response.json();
      if (result.status !== 'success') {
        console.error('Failed to update CDS config:', result.message);
      ...
      console.error('Error updating CDS config:', error);
  ...
  async evaluatePatient(patientContext) {
    try {
      console.log('CDS API: Starting evaluatePatient for patient', patientContext?.patientId);
      ...
      if (cachedResult) {
        console.log('CDS API: Returning cached result');
        return cachedResult;
      }
      ...
      if (this._inFlightEvaluations.has(cacheKey)) {
        console.log('CDS API: Reusing in-flight evaluation request for cacheKey', cacheKey);
      ...
      const postEvaluate = async (actionName) => {
        ...
        console.log(`CDS API: Making POST request to backend (action=${actionName})`);
        const response = await this.fetchWithRetry(this.baseUrl, {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' },
        ...
        console.log('CDS API: Received response, status:', response.status);
        const parsed = await response.json();
        console.log('CDS API: Parsed response:', parsed);
  ...
  try {
        ...
        if (result.status !== 'success') {
          console.error('CDS API: Failed to evaluate patient data:', result.message);
          return {
            version: 'error',
            warnings: [],
            prompts: [{
        ...
        console.log('CDS API: Evaluation completed successfully');
        return result.data;
      })();
      ...
      console.error('Error evaluating patient data:', error);
      return {
        version: 'error',
        warnings: [],
        prompts: [{
  ...
  async getKnowledgeBaseMetadata() {
    try {
      ...
      const result = await response.json();
      if (result.status !== 'success') {
        console.error('Failed to get KB metadata:', result.message);
        return null;
      }
  ...
  async logEvents(events) {
    try {
      ...
      if (!this.isOnline) {
        console.log('Cannot log events: offline mode');
        return false;
      }
      ...
      try {
        ...
        console.warn('logEvents fetch failed, attempting form POST fallback:', err && err.message ? err.message : err);
        try {
          ...
          console.error('logEvents form POST fallback failed:', formErr);
          return false;
        }
      }
    } catch (error) {
      console.error('Error logging CDS events:', error);
      return false;
    }
  }
