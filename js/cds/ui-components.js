...
if (typeof window.CDSIntegration !== 'undefined') {
  ...
  try {
    ...
    header.innerHTML = `
      ...
      <div class="cds-version">Version ${analysis.version}</div>
      <div class="cds-actions">
        <button id="openHighRiskDashboardBtn" class="btn btn-sm btn-outline-primary">High-Risk Dashboard</button>
      </div>
    `;
    ...
    contentContainer.className = 'cds-recommendations-content';
    cdsRecommendationsContainer.appendChild(contentContainer);
    ...
    if (analysis.plan && (analysis.plan.monotherapySuggestion || analysis.plan.addonSuggestion || analysis.plan.referral)) {
      // Check if we should suppress plan due to adherence gating
      ...
      planSection.className = 'cds-section plan-summary';
      const planTitle = document.createElement('h4');
      planTitle.textContent = 'Suggested Plan';
      ...
      if (analysis.plan.addonSuggestion) {
        ...
        const strong = document.createElement('strong');
        strong.textContent = 'Add-on suggestion:';
      ...
      contentContainer.appendChild(planSection);
    }
    
    // Check for adherence gating alerts - if present, suppress optimization prompts
    ...
    let filteredPrompts = analysis.prompts || [];
    let filteredDoseFindings = analysis.doseFindings || [];
    
    if (hasAdherenceGating) {
      // Suppress optimization/change prompts when adherence is gating
      filteredPrompts = filteredPrompts.filter(p => 
        // Keep prompts that are NOT optimization-related or are adherence-focused
        !(p.id && (
          p.id.includes('doseOptimization') || 
          p.id.includes('escalate') || 
          p.id.includes('switch') ||
          p.id.includes('addon') ||
          p.id.includes('monotherapy') ||
          p.id.includes('titration')
        )) &&
        !(p.text && (
          p.text.toLowerCase().includes('optimize') ||
          p.text.toLowerCase().includes('increase dose') ||
          p.text.toLowerCase().includes('add medication') ||
          p.text.toLowerCase().includes('switch medication')
        ))
      );
      
      // Also filter dose findings to suppress optimization recommendations
    ...
    // 1. Add treatment recommendations section (filtered)
    if (filteredPrompts && filteredPrompts.length > 0) {
      this.renderTreatmentRecommendationsSection(contentContainer, filteredPrompts);
    }
    ...
    // 3. Add warnings section (high severity alerts)
    if (analysis.warnings && analysis.warnings.length > 0) {
      this.renderWarningsSection(contentContainer, analysis.warnings);
    }
    
    // 4. Add dose findings section (filtered)
    ...
    // Add adherence gating notice if applicable
    if (hasAdherenceGating) {
      const gatingNotice = document.createElement('div');
      gatingNotice.className = 'cds-section adherence-gating-notice';
      gatingNotice.innerHTML = `
        <div class="alert alert-warning">
          <h5><i class="fas fa-exclamation-triangle"></i> Adherence Priority</h5>
          <p>Optimization and treatment change recommendations are currently suppressed due to adherence concerns. 
          Please address adherence barriers first before considering medication changes.</p>
        </div>
      `;
      contentContainer.insertBefore(gatingNotice, contentContainer.firstChild);
    }
        ...
        btn.addEventListener('click', () => {
          if (typeof window.cdsIntegration !== 'undefined' && typeof window.cdsIntegration.openHighRiskDashboard === 'function') {
            ...
            if (window.makeAPICall) {
              ...
              }).catch(e => console.error('Failed to fetch high-risk report', e));
            }
          }
        });
      }
    } catch (e) { console.warn('High-risk dashboard wiring failed', e); }
    
  } catch (error) {
    console.error('Error rendering enhanced CDS output:', error);
  }
};
}
...
CDSIntegration.prototype.renderHighRiskModal = function(reportRows) {
  try {
      ...
      modal.innerHTML = `
        <div class="cds-modal-content">
          <div class="cds-modal-header">
            ...
            <button id="closeHighRiskModal" class="btn btn-sm">Close</button>
          </div>
    ...
    if (!reportRows || reportRows.length === 0) {
      body.textContent = 'No high-risk patients detected.';
      return;
    }
  ...
  } catch (e) { console.error('renderHighRiskModal failed:', e); }
};

// Add to CDSIntegration class prototype
CDSIntegration.prototype.snoozeAlert = function(alertId) {
  if (!alertId) return false;
  this.snoozedAlerts.add(alertId);
  
  // Record telemetry
  this.telemetry.recordEvent('cds_action', {
    action: 'snooze',
    ruleId: alertId,
...
CDSIntegration.prototype.acknowledgeAlert = function(alertId) {
  if (!alertId) return false;
...
CDSIntegration.prototype.clearSnoozedAlerts = function() {
  this.snoozedAlerts.clear();
};
  ...
  if (!analysis.success) {
    container.innerHTML += `
      <div class="cds-error">
        ...
        <span>CDS evaluation failed: ${analysis.error || 'Unknown error'}</span>
      </div>
    `;
    return;
  }
  ...
  if (analysis.warnings.length === 0 && analysis.prompts.length === 0) {
    container.innerHTML += `
      <div class="cds-no-alerts">
        ...
        <span>No clinical alerts or recommendations at this time</span>
      </div>
    `;
  }
  ...
  alertEl.innerHTML = `
    <div class="alert-header">
      ...
      <div class="alert-actions">
        <button class="action-btn snooze-btn" title="Snooze for this session">
          <i class="fas fa-bell-slash"></i>
        </button>
        <button class="action-btn ack-btn" title="Acknowledge and dismiss">
          <i class="fas fa-check"></i>
        </button>
      </div>
    </div>
    ${alert.rationale ? `<div class="alert-rationale">${alert.rationale}</div>` : ''}
    ${alert.references ? `<div class="alert-references">${alert.references}</div>` : ''}
 ...
 */
CDSIntegration.prototype.renderTreatmentRecommendationsSection = function(container, recommendations) {
  const section = document.createElement('div');
  section.className = 'cds-section treatment-recommendations';
 ...
 */
CDSIntegration.prototype.renderWarningsSection = function(container, warnings) {
  if (warnings.length === 0) return;
  const section = document.createElement('div');
  section.className = 'cds-section warnings';
  const sectionTitle = document.createElement('h4');
  ...
  warnings.forEach(warning => {
    ...
    // Next steps (bulleted)
    if (warning.nextSteps && warning.nextSteps.length > 0) {
      const nextSteps = document.createElement('ul');
