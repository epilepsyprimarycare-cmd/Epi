...
class CDSGovernance {
  ...
  async loadConfiguration() {
    ...
    try {
      ...
      if (backendConfig) {
        ...
        console.log('CDS governance loaded from backend:', backendConfig);
        return;
      }
    } catch (error) {
      console.warn('Failed to load CDS config from backend:', error);
    }
    ...
    try {
      ...
      if (savedConfig) {
        ...
        console.log('CDS governance loaded from localStorage cache');
      }
    } catch (error) {
      console.warn('Failed to load CDS governance configuration:', error);
    } finally {
      this.isLoading = false;
    }
  }
  ...
  async fetchKnowledgeBaseMetadata() {
    try {
      console.log('CDS governance: Fetching knowledge base metadata...');
      ...
      try {
        ...
        if (result.status === 'success') {
          console.log('CDS governance: KB metadata loaded successfully');
          return result.data;
        }
        throw new Error(result.message || 'KB metadata fetch failed');
      } catch (error) {
        console.warn('Failed to fetch KB metadata via fetch, returning null:', error);
        return null;
      }
    } catch (error) {
      console.warn('Failed to fetch KB metadata:', error);
      return null;
    }
  }
  ...
  async fetchBackendConfig() {
    try {
      console.log('CDS governance: Fetching backend configuration...');
      ...
      try {
        ...
        if (result && result.status === 'success') {
          console.log('CDS governance: Backend configuration loaded successfully');
          return result.data;
        }
        console.log('CDS governance: Backend config fetch returned no valid data:', result);
        return null;
      } catch (error) {
        console.warn('CDS governance: Backend config fetch failed, using local defaults:', error.message);
        return null;
      }
    } catch (error) {
      console.warn('CDS governance: Backend config fetch failed, using local defaults:', error.message);
  ...
  async saveBackendConfiguration(updates = {}) {
    try {
      ...
      if (result && result.status === 'success') {
        ...
        console.log('CDS configuration saved to backend:', result.data);
        return true;
      } else {
        throw new Error(result.message || 'Backend config save failed');
      }
    } catch (error) {
      console.error('Backend config save error:', error);
      return false;
    }
  }
  ...
  saveLocalConfiguration() {
    try {
      ...
      console.warn('Failed to save CDS governance configuration to localStorage:', error);
    }
  }
  ...
  async getCurrentKBVersion() {
    try {
      ...
      console.warn('Could not fetch current KB version:', error);
  ...
  logAuditEvent(eventType, eventData) {
    ...
    this.saveConfiguration();
    console.log('CDS Audit:', auditEntry);
  }
