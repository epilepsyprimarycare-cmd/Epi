    ...
    try {
        if (typeof window !== 'undefined' && window.cdsApi && typeof window.cdsApi.getKnowledgeBaseMetadata === 'function') {
            ...
            if (meta && meta.formulary) {
                ...
                console.log('Dose adequacy: loaded canonical formulary from CDS KB (backend)');
                return CDS_CANONICAL_FORMULARY;
            }
        }
    } catch (e) {
        console.warn('Dose adequacy: failed to load canonical formulary from backend KB', e);
    }
...
function checkDoseAdequacy(medicationKey, doseOption, weightKg, age = 25) {
    ...
    if (unit.indexOf('mg/kg') !== -1) {
        ...
        if (opt != null && mgPerKg >= min && mgPerKg < opt) return 'suboptimal';
        if (opt != null && mgPerKg >= opt && mgPerKg <= max) return 'adequate';
        ...
        if (opt != null && dailyMg >= min && dailyMg < opt) return 'suboptimal';
        if (opt != null && dailyMg >= opt && dailyMg <= max) return 'adequate';
...
function updateMedicationDropdownHighlighting(medicationKey, selectElement, weightKg, age = 25) {
    ...
    console.log('Updating highlighting for', medicationKey, 'weight:', weightKg);
...
function updateAllMedicationDropdowns(formElement, weightKg, age = 25) {
    if (!formElement) {
        console.log('No form element provided');
        return;
    }
    
    console.log('Updating all medication dropdowns in form, weight:', weightKg, 'form ID:', formElement.id);
...
try {
    if (typeof window !== 'undefined') {
        ...
        initCanonicalFormulary().catch(err => console.warn('initCanonicalFormulary error:', err));
    }
} catch (err) {
    console.warn('Failed to schedule canonical formulary init:', err);
}
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        console.warn('Dose adequacy init error:', e);
    }
});