...
function getLatestFollowUpForPatient(patientId, followUps) {
    ...
    matches.sort((a, b) => {
        const dateA = (typeof parseDateFlexible === 'function') ? parseDateFlexible(a.FollowUpDate || a.followUpDate || a.SubmissionDate || a.submissionDate) : new Date(a.FollowUpDate || a.followUpDate || a.SubmissionDate || a.submissionDate || 0);
        const dateB = (typeof parseDateFlexible === 'function') ? parseDateFlexible(b.FollowUpDate || b.followUpDate || b.SubmissionDate || b.submissionDate) : new Date(b.FollowUpDate || b.followUpDate || b.SubmissionDate || b.submissionDate || 0);
...
const NotificationManager = (function() {
    ...
    async function subscribeUserToPush(registration) {
        try {
            ...
            window.Logger.debug('[ServiceWorker] Push subscription successful: ', subscription);
    ...
    async function sendSubscriptionToServer(subscription) {
        ...
        window.Logger.debug("Attempting to send subscription for PHC:", window.currentUserPHC);
...
window._debugPatientFollowUp = function(patientId) {
    ...
    console.log('window.allFollowUps length:', (window.allFollowUps || []).length);
    console.log('window.followUpsData length:', (window.followUpsData || []).length);
    
    const latest = getLatestFollowUpForPatient(patientId);
    console.log('Latest follow-up found:', latest);
    ...
    if (latest) {
        ...
        console.log('Formatted:', parsed ? formatDateForDisplay(parsed) : 'N/A');
    }
    ...
    console.log('All follow-ups for patient:', allMatches);
    console.groupEnd();
