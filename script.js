...
// Forward declaration: window.showPatientDetails
// This ensures the function is available globally even before the full definition loads.
// The actual implementation is defined later in this file and will override this.
if (typeof window.showPatientDetails !== 'function') {
    window.showPatientDetails = function(patientId, _retryCount) {
        const retryCount = _retryCount || 0;
        const maxRetries = 5;
        
        // Check if real implementation is available
        if (typeof window._showPatientDetailsImpl === 'function') {
            window._showPatientDetailsImpl(patientId);
            return;
        }
        
        // If called before the real function is ready, retry with increasing delay
        if (retryCount < maxRetries) {
            const delay = 300 + (retryCount * 200); // 300ms, 500ms, 700ms, 900ms, 1100ms
            if (retryCount === 0) {
                console.warn('[APP] showPatientDetails called before fully initialized, retrying...');
            }
            setTimeout(() => {
                window.showPatientDetails(patientId, retryCount + 1);
            }, delay);
        } else {
            console.error('[APP] showPatientDetails not available after retries');
            if (typeof showNotification === 'function') {
                showNotification('Please wait for the page to fully load and try again.', 'warning');
            }
        }
    };
}
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('[APP] DOMContentLoaded event fired in script.js');
...
function openSeizureClassifierModal() {
    console.log('[SEIZURE] openSeizureClassifierModal() called');
    const modal = document.getElementById('seizureClassifierModal');
    if (!modal) {
        console.error('[SEIZURE] Modal element not found with id="seizureClassifierModal"');
        window.Logger.error('script.js: Seizure classifier modal not found');
        return;
    }
    console.log('[SEIZURE] Modal element found:', modal);
    ...
    console.log('[SEIZURE] Modal display set to flex');
    ...
    if (typeof initializeSeizureClassifier === 'function') {
        ...
        console.log('[SEIZURE] Calling initializeSeizureClassifier with patientId:', window.currentPatientId, 'ageAtOnset:', ageAtOnset);
        ...
        console.log('[SEIZURE] initializeSeizureClassifier completed');
    } else {
        console.error('[SEIZURE] initializeSeizureClassifier function not found');
    }
    
    window.Logger.debug('script.js: Opened seizure classifier modal for patient', window.currentPatientId);
    console.log('[SEIZURE] openSeizureClassifierModal() complete');
}
...
function openSeizureClassifierModalForForm(targetFieldId) {
    console.log('[SEIZURE] openSeizureClassifierModalForForm() called with targetFieldId:', targetFieldId);
    const modal = document.getElementById('seizureClassifierModal');
    if (!modal) {
        console.error('[SEIZURE] Modal element not found with id="seizureClassifierModal"');
        window.Logger.error('script.js: Seizure classifier modal not found');
    ...
    if (typeof initializeSeizureClassifierForForm === 'function') {
        ...
        console.log('[SEIZURE] Calling initializeSeizureClassifierForForm with targetFieldId:', targetFieldId, 'ageAtOnset:', ageAtOnset);
        initializeSeizureClassifierForForm(targetFieldId, ageAtOnset);
        console.log('[SEIZURE] initializeSeizureClassifierForForm completed');
    } else {
        // Fallback if function not available yet
        console.error('[SEIZURE] initializeSeizureClassifierForForm function not found');
        window.Logger.warn('initializeSeizureClassifierForForm not found');
        if (typeof initializeSeizureClassifier === 'function') {
            console.log('[SEIZURE] Fallback: Calling initializeSeizureClassifier with "new_patient"');
            initializeSeizureClassifier('new_patient');
        } else {
            console.error('[SEIZURE] Fallback failed: initializeSeizureClassifier function also not found');
        }
    }
    
    window.Logger.debug('script.js: Opened seizure classifier modal for form field', targetFieldId);
    console.log('[SEIZURE] openSeizureClassifierModalForForm() complete');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('[SEIZURE] DOMContentLoaded: Deferring seizure helper initialization to ensure functions are defined');
    setTimeout(() => {
        console.log('[SEIZURE] setTimeout tick: Checking if initializeSeizureHelperButtons is available');
        if (typeof initializeSeizureHelperButtons === 'function') {
            console.log('[SEIZURE] Found initializeSeizureHelperButtons, calling it directly');
            initializeSeizureHelperButtons();
        } else {
            console.log('[SEIZURE] initializeSeizureHelperButtons still not available, using retry mechanism');
            ...
            helperButtons.forEach(btn => {
                ...
                btn.addEventListener('click', (e) => {
                    try {
                        ...
                        if (!targetFieldId) {
                            console.error('[SEIZURE] fallback: No data-seizure-helper-target on button');
                        ...
                        const iv = setInterval(() => {
                            ...
                            if (callOpener()) {
                                ...
                                clearInterval(iv);
                                console.error('[SEIZURE] fallback: opener functions still not available after wait');
                        ...
                        console.error('[SEIZURE] fallback: no opener function available');
                        ...
                        console.error('[SEIZURE] fallback click handler error', inner);
                    }
                });
                btn.dataset.fallbackBound = 'true';
            });
            console.log('[SEIZURE] fallback binding executed for', helperButtons.length, 'buttons');
        } catch (e) {
            console.warn('[SEIZURE] fallback binding failed', e);
        }
    };
        ...
        btn.addEventListener('click', async () => {
            ...
            try {
                if (target === 'mg-users') {
                    ...
                    const helper = (typeof window.renderCdsRulesList === 'function')
                        ? window.renderCdsRulesList
                        : await waitForManagementHelper('renderCdsRulesList');
                    if (helper) {
                        await helper();
                    } else {
                        window.Logger.warn('renderCdsRulesList helper missing');
                        const el = document.getElementById('cdsRulesContainer');
                        if (el) {
                            el.innerHTML = '<div class="alert alert-warning">CDS rules viewer is initializing. Please try again shortly.</div>';
                        }
                    }
...
function ensureSeizureHelperTriggersBound(maxAttempts = 100, intervalMs = 50) {
    let attempts = 0;
    console.log('[SEIZURE] ensureSeizureHelperTriggersBound() called - will retry up to', maxAttempts, 'times');

    const tryBind = () => {
        if (typeof initializeSeizureHelperButtons === 'function') {
            console.log('[SEIZURE] ensureSeizureHelperTriggersBound: Found initializeSeizureHelperButtons on attempt', attempts);
            initializeSeizureHelperButtons();
            return true;
        }
        return false;
    ...
    if (tryBind()) {
        console.log('[SEIZURE] ensureSeizureHelperTriggersBound: Immediate success');
        return;
    }
    ...
    const timer = setInterval(() => {
        attempts += 1;
        if (tryBind()) {
            console.log('[SEIZURE] ensureSeizureHelperTriggersBound: Success after', attempts, 'attempts');
            clearInterval(timer);
        } else if (attempts >= maxAttempts) {
            clearInterval(timer);
            console.error('[SEIZURE] ensureSeizureHelperTriggersBound: Failed after', maxAttempts, 'attempts - function not found');
            window.Logger && window.Logger.warn && window.Logger.warn('Seizure helper initializer unavailable after waiting');
        }
    }, intervalMs);
}
...
function updateTabVisibility() {
    ...
    if (canAccessManagement) {
        ...
        const restrictedSubtabs = ['mg-facilities', 'mg-analytics', 'mg-cds', 'mg-logs'];
    ...
    if (tabName === 'management' && currentUserRole === 'master_admin') {
        ...
        try {
            ...
            (async () => {
                ...
                } catch (e) { window.Logger.warn('initUsersManagement (dynamic) failed', e); }
            })();
        } catch (e) {
            window.Logger.warn('Failed to initialize Management default subtab:', e);
        }
    }
...
function updateKPIGauges() {
    ...
    console.log('[Adherence] Active patients:', activePatients.length);
    console.log('[Adherence] All follow-ups:', Array.isArray(followUpsData) ? followUpsData.length : 0);
    console.log('[Adherence] Relevant follow-ups:', relevantFollowUps.length);
    console.log('[Adherence] Sample follow-up TreatmentAdherence values:', relevantFollowUps.slice(0, 5).map(f => f.TreatmentAdherence));
    ...
    if (relevantFollowUps.length > 0) {
        ...
        console.log('[Adherence] Unique patients with follow-ups:', latestFollowUpsList.length);
        ...
        console.log('[Adherence] Patients with good adherence:', patientsWithGoodAdherence);
    } else {
        // If no follow-up data available, assume 0% good adherence
        patientsWithGoodAdherence = 0;
    }

    // Calculate adherence rate based on patients who have follow-up data with adherence info
    // Use latestFollowUps count as denominator (patients with adherence data) instead of all active patients
    ...
    console.log('[Adherence] Patients with adherence data:', patientsWithAdherenceData);
    console.log('[Adherence] Adherence rate:', adherenceRate + '%');
...
function renderGauge(containerId, value, colorStops) {
    console.log(`[renderGauge] Rendering gauge '${containerId}' with value:`, value);
    try {
        const canvas = document.getElementById(containerId);

        // Check if the element exists and is a canvas
        if (!canvas || canvas.tagName !== 'CANVAS') {
            console.warn(`[renderGauge] Cannot render gauge: Element with ID '${containerId}' is not a valid canvas`);
            return null;
        }
        ...
        if (!ctx) {
            console.warn(`[renderGauge] Cannot render gauge: Failed to get 2D context for '${containerId}'`);
            return null;
        }
...
function renderPatientListFromArray(array, startIndex = 0, searchTerm = '', appendToExisting = false) {
    ...
    page.forEach(p => {
        ...
        patientCard.addEventListener('click', () => window.showPatientDetails(normalizedPatient.ID));
...
function initializeAllCharts() {
    ...
    // renderTreatmentSummaryTable is deprecated - removed to prevent errors
...
function renderFollowUpTrendChart() {
    console.log('[FollowUpTrendChart] Starting render. Total followUps:', (Array.isArray(followUpsData) ? followUpsData.length : 0));
    console.log('[FollowUpTrendChart] followUpsData sample:', Array.isArray(followUpsData) ? followUpsData.slice(0, 2) : 'not an array');
    ...
    console.log('[FollowUpTrendChart] Chart data:', chartData);
...
function renderPHCFollowUpMonthlyChart() {
        ...
        if (!chartEl) {
            ...
            chartBoxes.forEach(box => {
                if (box.querySelector('h3')?.textContent.includes('Monthly') || 
                    box.querySelector('.no-data-message')?.textContent.includes('Follow-ups by PHC')) {
                    chartContainer = box;
                }
            });
            if (chartContainer) {
    ...
    if (!activePatients || activePatients.length === 0) {
        ...
        showNoDataMessage('No Data Available for Monthly Follow-ups by PHC');
        return;
    }
    ...
    if (phcLabels.length === 0) {
        ...
        showNoDataMessage('No PHC Data Available for Monthly Follow-ups');
        return;
    }
    ...
    phcLabels.forEach(phc => {
        ...
        // Completed: FollowUpStatus contains 'Completed' (case-insensitive)
        const completed = patientsInPhc.filter(p => 
            p.FollowUpStatus && p.FollowUpStatus.toLowerCase().includes('completed')
        ).length;
        // Pending/Left: FollowUpStatus is exactly 'Pending' (case-insensitive)
        const pending = patientsInPhc.filter(p => 
            p.FollowUpStatus && p.FollowUpStatus.toLowerCase() === 'pending'
        ).length;
    ...
    window.Logger && window.Logger.debug && window.Logger.debug('[PHCFollowUpMonthlyChart] Pending:', pendingCounts);
...
function renderProcurementForecast() {
    try {
        ...
        window.Logger.debug('renderProcurementForecast: Selected PHC:', selectedPhc);
...
function renderTreatmentSummaryTable_OLD() {
    ...
    if (!phcFilterElement) {
        window.Logger.warn('treatmentSummaryPhcFilter element not found, using "All" as default');
        return;
    }
    const selectedPhc = phcFilterElement.value || 'All';
    const allActivePatients = getActivePatients();
    ...
    window.Logger.debug('renderTreatmentSummaryTable: Selected PHC:', selectedPhc);
...
// Expose showPatientDetails globally - overrides the forward declaration placeholder
window.showPatientDetails = showPatientDetails;
window._showPatientDetailsImpl = showPatientDetails; // Backup reference for forward declaration fallback
...
function attachLogoutButton() {
    console.log('[Logout] attachLogoutButton() called');
    try {
        const btn = document.getElementById('logoutBtn');
        console.log('[Logout] Button element:', btn);
        
        if (!btn) {
            console.warn('[Logout] Button not found with ID: logoutBtn');
            return;
        }
        ...
        const logoutHandler = function(event) {
            ...
            try {
                if (typeof window.logout === 'function') {
                    console.log('[Logout] Calling window.logout()...');
                    window.logout();
                } else {
                    console.error('[Logout] window.logout is not defined, forcing reload');
                    window.location.reload();
                }
            } catch (err) {
                console.error('[Logout] Logout handler failed; forcing full refresh.', err);
                window.location.reload();
            }
        };
        ...
        console.log('[Logout] Logout button handler attached successfully');
        
    } catch (err) {
        console.error('[Logout] Failed to wire logout button:', err);
    }
}
...
console.log('[Logout] Script loaded, document.readyState:', document.readyState);

if (document.readyState === 'loading') {
    console.log('[Logout] Waiting for DOMContentLoaded...');
    document.addEventListener('DOMContentLoaded', () => {
        console.log('[Logout] DOMContentLoaded fired');
        attachLogoutButton();
    });
} else {
    console.log('[Logout] DOM already loaded, attaching immediately');
    attachLogoutButton();
}

// Also attach after a short delay to ensure dashboard is rendered
setTimeout(() => {
    console.log('[Logout] Re-attempting attachment after 1 second delay...');
    attachLogoutButton();
}, 1000);
...
const HANDLERS = {
    ...
    exportAllData: typeof exportAllData === 'function' ? exportAllData : (window.exportAllData || null)
};
...
function attachGlobalActionListeners() {
    ...
    const map = [
        ...
        ['exportAllDataBtn', 'exportAllData']
    ];
    ...
    function safeCallByName(name, ...args) {
        try {
            ...
            window.Logger.warn(`Error calling handler ${name}:`, e);
        }
    }
...
window.downloadAllPatientsCsv = downloadAllPatientsCsv;
...
function initializeSeizureHelperButtons() {
    console.log('[SEIZURE] initializeSeizureHelperButtons() called');
    const helperButtons = document.querySelectorAll('[data-seizure-helper-target]');
    console.log('[SEIZURE] Found', helperButtons.length, 'elements with data-seizure-helper-target attribute');
    helperButtons.forEach((btn, idx) => {
        console.log(`[SEIZURE] Button ${idx}:`, btn.id, btn.getAttribute('data-seizure-helper-target'));
    });
    ...
    console.log('[SEIZURE] Successfully bound', boundCount, 'buttons directly');

    if (!seizureHelperDelegationBound) {
        console.log('[SEIZURE] Attaching event delegation listener to document');
        ...
        console.log('[SEIZURE] Event delegation listener already attached');
    }

    seizureHelperLog(`Seizure helper triggers detected: ${helperButtons.length}, newly bound: ${boundCount}`);
    console.log('[SEIZURE] initializeSeizureHelperButtons() complete - triggers ready:', helperButtons.length);
}
window.initializeSeizureHelperButtons = initializeSeizureHelperButtons;
console.log('[SEIZURE] Exported initializeSeizureHelperButtons to window object');

if (document.readyState !== 'loading') {
    // DOM already ready (module loaded late), bind immediately
    console.log('[SEIZURE] DOM already loaded, calling initializeSeizureHelperButtons immediately');
    initializeSeizureHelperButtons();
} else {
    console.log('[SEIZURE] DOM not loaded yet, registering DOMContentLoaded listener for initializeSeizureHelperButtons');
...
function bindSeizureHelperButton(button) {
    if (!button || button.dataset.helperBound === 'true') {
        console.log('[SEIZURE] Button already bound or invalid:', button);
        return false;
    }
    console.log('[SEIZURE] Binding seizure helper button:', button.id);

    button.addEventListener('click', event => {
        console.log('[SEIZURE] Button clicked:', button.id);
        if (button.disabled || button.getAttribute('aria-disabled') === 'true' || button.dataset.seizureHelperDisabled === 'true') {
            console.log('[SEIZURE] Button is disabled, ignoring click');
            return;
        }

        const targetFieldId = button.getAttribute('data-seizure-helper-target');
        console.log('[SEIZURE] Target field ID from button:', targetFieldId);
        if (!targetFieldId) {
            console.error('[SEIZURE] No data-seizure-helper-target attribute found');
        ...
        event.preventDefault();
        event.stopPropagation();
        console.log('[SEIZURE] Launching seizure helper flow for target:', targetFieldId);
    ...
    console.log('[SEIZURE] Button binding complete for:', button.id);
    return true;
}
...
function handleSeizureHelperTrigger(event) {
    ...
    if (!trigger) {
        console.log('[SEIZURE] No seizure helper trigger found in event');
        return;
    }
    console.log('[SEIZURE] Seizure helper trigger detected:', trigger.id, trigger);

    if (trigger.disabled || trigger.getAttribute('aria-disabled') === 'true' || trigger.dataset.seizureHelperDisabled === 'true') {
        console.log('[SEIZURE] Trigger is disabled, ignoring');
        return;
    }

    const targetField = trigger.getAttribute('data-seizure-helper-target');
    console.log('[SEIZURE] Target field from delegated trigger:', targetField);
    if (!targetField) {
        console.error('[SEIZURE] No data-seizure-helper-target found on delegated trigger');
    ...
    if (trigger.tagName === 'BUTTON' || trigger.tagName === 'A') {
        event.preventDefault();
        console.log('[SEIZURE] Prevented default behavior for', trigger.tagName);
    }

    console.log('[SEIZURE] Launching seizure helper flow from delegation for target:', targetField);
...
function launchSeizureHelperFlow(targetFieldId) {
    console.log('[SEIZURE] launchSeizureHelperFlow() called with targetFieldId:', targetFieldId);
    if (!targetFieldId) {
        console.error('[SEIZURE] No targetFieldId provided to launchSeizureHelperFlow');
        reportMissingTarget('launch flow');
        return;
    }

    if (typeof openSeizureClassifierModalForForm === 'function') {
        console.log('[SEIZURE] openSeizureClassifierModalForForm is available, calling it');
        openSeizureClassifierModalForForm(targetFieldId);
    } else if (typeof openSeizureClassificationHelper === 'function') {
        console.log('[SEIZURE] openSeizureClassifierModalForForm not found, using fallback openSeizureClassificationHelper');
        openSeizureClassificationHelper(targetFieldId);
    } else {
        console.error('[SEIZURE] Neither openSeizureClassifierModalForForm nor openSeizureClassificationHelper is available');
        reportHelperUnavailable();
    }
    console.log('[SEIZURE] launchSeizureHelperFlow() complete');
}
